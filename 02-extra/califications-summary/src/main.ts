...
const studentAverage : StudentSumary[]= [];
let studentSumary : StudentSumary = {
  name: '',
  highestCalification: 0,
  averageCalifications: ''
};

const sumarizeClassRoom = (studentList : Student[]) => {
  studentList.forEach((student) => {
    const number = Math.max.apply(Math, student.califications);
    const gradesSum = student.califications.reduce((pre, curr) => (curr += pre));
    const average = (gradesSum / student.califications.length).toFixed(2);
